...
While a Naive forecasting method forecasts a value equal to the one last observed, a Seasonal Naive method accounts for seasonality in the data by forecasting a value equal to the last value from the same season.
...
outbound_monthly_hrs_HW <- HoltWinters(outbound_summary_monthly_hrs_ts, seasonal = "multiplicative")
hw5 <- autoplot(forecast(outbound_monthly_hrs_HW, h = 24), main = "Holt-Winters Forecast, Outbound Monthly Hours", ylab = "Total Hours")

# Pay
outbound_monthly_pay_HoltWinters <- HoltWinters(outbound_summary_monthly_pay_ts, seasonal = "multiplicative")
...
The forecasts for the outbound series do not appear to provide as drastic of a forecast, as the forecast seems to more closely match the patterns that we have noticed from 2015 to 2017.
